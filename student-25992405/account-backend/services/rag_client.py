...
def _normalise_k(k):
    if isinstance(k, bool):
        raise RAGClientError("k must be a positive integer.")
    try:
        k = int(k)
    except (TypeError, ValueError) as exc:
        raise RAGClientError("k must be a positive integer.") from exc
    if k < 1 or k > 20:
        raise RAGClientError("k must be between 1 and 20.")
    return k
...
def _post(path, payload):
    ...
    try:
        ...
        try:
            ...
            raise RAGClientError(f"RAG service returned invalid JSON (HTTP {response.status_code}).") from exc
        if not response.ok:
...
def retrieve_context(query, k=5):
    return _post("/rag/retrieve", {"query": query, "k": _normalise_k(k)})


def answer_question(query, k=5):
    return _post("/rag/answer", {"query": query, "k": _normalise_k(k)})
